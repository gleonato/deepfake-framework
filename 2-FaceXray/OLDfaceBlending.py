...
def main():
    ...
    for i, srcFace in enumerate(srcFaces):
        # load bgr
        print('Loading Source Faces...')
        try:
            srcFaceBgr = cv2.imread(srcFace)
            print('Source Face {} loaded'.format(srcFace))
        except:
        ...
        saved = cv2.imwrite(f'./results/mask_{i}.jpg', hullMask)
        if saved is True:
...
def get_landmarks(detector, predictor, rgb):
    ...
    for box in boxes:
        ...
        print('Landmarks found in {}'.format(box))
        break
    else:
        return None
...
def find_one_neighbor(detector, predictor, srcPath, srcLms, faceDatabase, threshold):
    for face in faceDatabase:
        ...
        print('Image {} Landmarks Euclidean distance: {}'.format(face,dist))
